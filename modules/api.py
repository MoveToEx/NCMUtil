...
class NCMAPI:
    ...
    def call_weapi(self, path, obj):
        path = path.removeprefix('/')
        text = json.dumps(obj, ensure_ascii=False)
        secretKey = ''.join(
            map(lambda x: Constant.base62[x % 62], get_random_bytes(16))).encode()
        data = {
            "params": b64encode(NCMAPI._aes(
                b64encode(NCMAPI._aes(text, Constant.presetKey,
                          Constant.iv)), secretKey, Constant.iv
            )).decode(),
            "encSecKey": NCMAPI._rsa(secretKey[::-1], Constant.publicKey)
        }
        res = self.session.post(
            'https://music.163.com/weapi/' + path, data=data)
    ...
    def search(self, keyword, offset=0, type=1, limit=10) -> dict:
        ...
        return self.call_weapi('cloudsearch/get/web', data)
    ...
    def detail(self, id: int | list[int]):
        ...
        return self.call_weapi('v3/song/detail', data)
    ...
    def lyric(self, id: int):
        ...
        return self.call_weapi('song/lyric/v1', data)

    def album(self, id: int):
        return self.call_weapi(f'v1/album/{id}', {})
